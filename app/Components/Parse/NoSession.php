    ...
    protected function parseBb($txt)
    {
        if (preg_match_all('#\[quote=([^]]*)]#i', $txt, $match)) {
            foreach ((array)$match[1] as $val) {
                $txt = str_replace(
                    '[quote=' . $val . ']',
                    '<blockquote class="wwblock"><cite>' . htmlspecialchars($val, ENT_QUOTES) . ' писал(а)</cite><div class="wwquote">',
                    $txt
                );
            }
        }
        ...
        if (preg_match_all('#\[color=([^]]*)]#i', $txt, $match)) {
            foreach ((array)$match[1] as $val) {
                $txt = str_replace(
                    '[color=' . $val . ']',
                    '<span style ="color:' . htmlspecialchars($val, ENT_QUOTES) . '">',
                    $txt
                );
            }
        }

        $txt = preg_replace_callback('#\[spoiler=%(.*?)%](.*?)\[/spoiler]#is', static function ($val) {
