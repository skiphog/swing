...
class All extends NoSession
{
    ...
    protected function parseUrl($txt)
    {
        if (preg_match_all('#\[url=([^]]*)]#i', $txt, $match)) {
            foreach ((array)$match[1] as $val) {
                $txt = str_replace(
                    '[url=' . $val . ']',
                    '<a href="' . htmlspecialchars($val, ENT_QUOTES | ENT_SUBSTITUTE) . '" target="_blank">',
                    $txt
                );
            }
        }
    ...
    protected function parseImg($txt)
    {
        if (preg_match_all('#\[(img)=?(.*?)](.+?)\[/img]#', $txt, $match)) {
            foreach ((array)$match[3] as $key => $val) {
                ...
                $txt = str_replace(
                    $match[0][$key],
                    '<img class="new-imgart" style="' . $param . '" src="' . htmlspecialchars($val, ENT_QUOTES | ENT_SUBSTITUTE) . '">',
                    $txt
                );
            }
        }
    ...
    {
        return preg_replace_callback('#\[video]([^[]*)\[/video]#i', static function ($item) {
