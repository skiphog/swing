...
use App\Components\Parse\GParse;
...
<?php $this->start('title') ?><?php echo e($diary['title_di']); ?><?php $this->stop() ?>
<?php $this->start('description') ?><?php echo e($diary['title_di']); ?><?php $this->stop() ?>

<?php $this->start('style') ?>
<style>
    .d-breadcrumbs,.d-body,.t-comment{margin-bottom:10px}
    .d-breadcrumbs{font-size:1.2em}
    .d-user-info{margin-bottom: 10px}
    ...
    .msg-avatar > img,.t-avatar > img,.t-uc-info > img{vertical-align:middle;padding:0}
    ...
    .diary-views{background: url(/img/spritesheet-5.png) no-repeat -2px -3px;padding-left: 22px;cursor: help;font-weight:700}
</style>
<?php $this->stop() ?>
...
<div id="mass-content">
    <div class="d-breadcrumbs">
        <a href="<?php echo url('/diaries'); ?>">Все дневники</a> &bull;
        <a href="/diaries/user/<?= $diary['id_user']; ?>">дневник <?php echo e($diary['login']); ?></a>
    </div>

    <div class="d-user-info">
        <div class="msg-avatar">
            <img class="border-box"
                src="<?php echo avatar($myrow, $diary['pic1'], $diary['photo_visibility']); ?>"
                width="60" height="60" alt="avatar" draggable="false">
        </div>
        <div>
            <a class="hover-tip" href="/user/<?php echo $diary['id_user']; ?>">
                <?php echo genderIcon($diary['gender']); ?> <?php echo e($diary['login']); ?>
            </a>
            <div class="date">
                <?php echo ($date = swDate($diary['data_di']))->format('d-m-Y'); ?> |
                <strong><?php echo $date->humans(); ?></strong>
            </div>
            <div>
                <span class="diary-views" title="Просмотры"><?php echo $diary['v_count']; ?></span>
                <?php if ($access) : ?>
                    <a href="/diaries/<?= $diary_id; ?>/edit">Редактировать</a> &bull;
                ...
                <div class="t-comment border-box">
                    ...
                    <div class="t-user-info">
                        <a class="hover-tip" href="/id<?php echo $value['id']; ?>" target="_blank">
                            <?php echo genderIcon($value['gender']); ?> <?php echo e($value['login']); ?>&nbsp;
                        </a>
                        <?php if ($myrow->isUser()) { ?>
				        ...
				        <?php echo date('d-m-Y H:i',
                            strtotime($value['comment_date'])); ?> | <strong><?php echo swDate($value['comment_date'])->humans(); ?></strong>
			        </span>
                    </div>
